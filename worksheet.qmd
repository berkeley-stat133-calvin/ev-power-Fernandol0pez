...
```{r}
sources <- str_remove_all(energy_sources, "\\s*\$begin:math:text$.*\\$end:math:text$")
...
```{r}
energy_clean <- tibble(source = sources, unit = units)
energy_clean
```
...
```{r}
states_left <- left_join(states, regions, by = "state")
head(states_left)
```
...
```{r}
states_inner <- inner_join(states, regions, by = "state")
nrow(states_inner)
```
...
d.  Discussion: Why would you prefer a left join over an inner join when merging state-level datasets?\
     you'd pefer a left join over a inner join because you dont want to loose data even if region info is missing.
e.  Use pivot_longer() to make the socioeconomic data longer.\

```{r}
states_long <- states |>
  pivot_longer(cols = c(Income, Population, Illiteracy),
               names_to = "variable",
               values_to = "value")
...
ggplot(us_map_joined, aes(long, lat, group = group, fill = Income)) +
  ...
  labs(title = "Income by State", fill = "Income") +
  theme_minimal()
```
...
```{r}
# Add Life Exp column
state_data2 <- data.frame(state.x77) |>
  mutate(state = rownames(state.x77),
         state = tolower(state)) |>
  select(state, `Life Exp`)

us_map_joined2 <- us_map |>
  left_join(state_data2, by = c("region" = "state"))

ggplot(us_map_joined2, aes(long, lat, group = group, fill = `Life.Exp`)) +
  geom_polygon(color = "white") +
  scale_fill_gradient(low = "lightblue", high = "darkgreen") +
  labs(title = "Life Expectancy by State", fill = "Years") +
  theme_minimal()
```
